...
//   img.Image addNoise(img.Image image) {
//     var noisyImage = img.Image.from(image);
//     for (var y = 0; y < noisyImage.height; y++) {
//       for (var x = 0; x < noisyImage.width; x++) {
//         var pixel = noisyImage.getPixel(x, y);
//         var noise = math.Random().nextInt(30) - 15; // Noise range [-15, 15]
//         var r = (pixel.r + noise).clamp(0, 255);
//         var g = (pixel.g + noise).clamp(0, 255);
//         var b = (pixel.b + noise).clamp(0, 255);
//         noisyImage.setPixelRgba(x, y, r, g, b, pixel.a);
//       }
//     }
//     return noisyImage;
//   }
...
//     var contrastedImage = img.Image.from(image);
//     for (var y = 0; y < contrastedImage.height; y++) {
//       for (var x = 0; x < contrastedImage.width; x++) {
//         var pixel = contrastedImage.getPixel(x, y);
//         var r = ((pixel.r - 128) * factor + 128).clamp(0, 255);
//         var g = ((pixel.g - 128) * factor + 128).clamp(0, 255);
//         var b = ((pixel.b - 128) * factor + 128).clamp(0, 255);
//         contrastedImage.setPixelRgba(x, y, r, g, b, pixel.a);
//       }
//     }
//     return contrastedImage;
//   }

//   img.Image applyGammaCorrection(img.Image image, double gamma) {
//     var gammaCorrected = img.Image.from(image);
//     for (var y = 0; y < gammaCorrected.height; y++) {
//       for (var x = 0; x < gammaCorrected.width; x++) {
//         var pixel = gammaCorrected.getPixel(x, y);
//         var r = (255.0 * math.pow(pixel.r / 255.0, gamma)).clamp(0, 255);
//         var g = (255.0 * math.pow(pixel.g / 255.0, gamma)).clamp(0, 255);
//         var b = (255.0 * math.pow(pixel.b / 255.0, gamma)).clamp(0, 255);
//         gammaCorrected.setPixelRgba(
//             x, y, r.toInt(), g.toInt(), b.toInt(), pixel.a);
//       }
//     }
...
//           var noisyImage = addNoise(originalImage);
//           await _processAugmentedImage(noisyImage);

//           var contrastedImage = adjustContrast(originalImage, 1.5);
//           await _processAugmentedImage(contrastedImage);

//           var gammaCorrected = applyGammaCorrection(originalImage, 0.5);
//           await _processAugmentedImage(gammaCorrected);

//           // Horizontal and vertical flips
//           var flippedHorizontally = img.flipHorizontal(originalImage);
//           await _processAugmentedImage(flippedHorizontally);

//           var flippedVertically = img.flipVertical(originalImage);
//           await _processAugmentedImage(flippedVertically);
//         }
//       } else {
//         throw Exception(detectionResult.message);
//       }
...
class _RegistrationScreenState extends State<RegistrationScreen>
  ...
  CameraController? _cameraController;
  List<File> capturedImages = [];
  List<Map<String, dynamic>> faceEmbeddings = [];
  int currentStep = 0;
  final int totalSteps = 7;
  String _statusMessage = '';
  bool _isProcessing = false;
    ...
    try {
      ...
      if (detectionResult.isValid) {
        ...
        final List<double> currentEmbedding =
            await _recognitionService.getFaceEmbedding(imageFile);
        print(
            'Capture ${currentStep + 1} - Embeddings length: ${currentEmbedding.length}');
        ...
        if (currentStep >= totalSteps) {
          ...
          bool isVerified = await _verifyRegistration();
          if (isVerified) {
            await _processRegistration();
          } else {
            _resetRegistration();
            setState(() {
              _statusMessage =
                  'Face verification failed. Please try registration again.';
            });
          }
        } else {
          _showFaceGuide();
  ...
  Future<bool> _verifyRegistration() async {
    try {
      ...
      final testEmbeddings =
          await _recognitionService.getFaceEmbedding(imageFile);

      // Compare with captured embeddings
      bool isMatch = false;
      for (var embedding in faceEmbeddings) {
        double similarity = _recognitionService.calculateSimilarity(
          testEmbeddings,
          List<double>.from(embedding['values']),
        );
  ...
  Future<void> _processRegistration() async {
    ...
    try {
      // Verify minimum required images
      if (faceEmbeddings.length < totalSteps) {
        throw Exception(
            'Insufficient face images. Please capture all required images.');
      }

      print('Processing registration for: ${_nameController.text}');
      print('Number of embeddings: ${faceEmbeddings.length}');

      // Add additional data to embeddings
      for (var i = 0; i < faceEmbeddings.length; i++) {
        faceEmbeddings[i]['capture_index'] = i;
        faceEmbeddings[i]['quality_score'] =
            1.0; // You can add actual quality metrics
      }
      ...
      final user =
          await _databaseService.getUserById(_employeeIdController.text);
      final data = user.data() as Map<String, dynamic>;
      print('Successfully registered user: ${_nameController.text}');
      print('Stored embeddings count: ${data['embeddings'].length}');
                ...
                children: [
                  ...
                  if (_isShowingGuide)
                    AnimatedFaceGuide(
                      step: currentStep,
                      animation: _animation,
                      guideMessage: _getStepInstructions(),
                    ),
                ],
              )
            else
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text('5. Complete all $totalSteps required captures'),
              Text('6. Maintain good lighting throughout the process'),
              Text('7. Remove glasses if wearing any'),
              Text('8. Keep a neutral expression'),
            ],
          ),
        ),
        actions: [
...
class FaceGuidePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Draw face features for better guidance
    final eyeRadius = radius * 0.1;
    final leftEyeCenter =
        Offset(centerX - radius * 0.3, centerY - radius * 0.3);
    final rightEyeCenter =
    ...
    if (step > 0) {
      ...
      switch (step) {
        ...
        case 5: // Left turn
          ...
          _drawArrow(canvas, start, end, arrowPaint);
          _drawArc(canvas, centerX + radius * 1.1, centerY, radius * 0.3,
              arrowPaint);
          break;
        case 6: // Right turn
          start = Offset(centerX - radius * 0.8, centerY);
  ...
  void _drawArc(Canvas canvas, double x, double y, double radius, Paint paint) {
